...
$(function () {
    ...
    $('.btn-reinicio').click(function () {
        if ($('.btn-reinicio').text() === 'Iniciar') {
            ...
            clearInterval(movimientosIntervalos);
...
function iniciarJuego() {
    ...
    movimientosIntervalos = setInterval(realizarMovimientos, 1500);
}
...
function realizarMovimientos() {
    ...
    var contador;
    var nombreImagen;
    var nombreImagenSiguiente;
    var figurasMarcadas = iniciarFiguras();
    var cambios = false;
        ...
        for (var col = 0; col < 7; ++col) {
            if ((7 - col) > 2) {
                ...
                while ((7 - col) >= 3 && contador < (7 - col)) {
                    nombreImagenSiguiente = $('.col-' + (col + contador + 1)).children()[row];
                    nombreImagenSiguiente = $(nombreImagenSiguiente).prop('src').substring($(nombreImagenSiguiente).prop('src').length - 5);

                    if (nombreImagen !== nombreImagenSiguiente) {
                        break;
                    }
            ...
            if ((7 - row) > 2) {
                ...
                while ((7 - row) >= 3 && contador < (7 - row)) {
                    nombreImagenSiguiente = $('.col-' + (col + 1)).children()[row + contador];
                    nombreImagenSiguiente = $(nombreImagenSiguiente).prop('src').substring($(nombreImagenSiguiente).prop('src').length - 5);
